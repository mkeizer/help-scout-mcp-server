...
export function clientForUser(userId) {
    ...
    if (clientPool.size >= CLIENT_POOL_MAX) {
        ...
        if (oldest !== undefined) {
            clientPool.get(oldest)?.closePool().catch(() => { });
...
export function dropClientForUser(userId) {
    clientPool.get(userId)?.closePool().catch(() => { });
