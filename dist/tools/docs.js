...
import { ListDocsCategoriesInputSchema, ListDocsArticlesInputSchema, SearchDocsArticlesInputSchema, GetDocsArticleInputSchema, CreateDocsArticleInputSchema, UpdateDocsArticleInputSchema, DeleteDocsArticleInputSchema, } from '../schema/types.js';
export class DocsToolHandler {
    ...
    getCollectionId(provided) {
        const id = provided || config.docs.collectionId;
        if (!id) {
            throw new Error('No collection ID provided. Set HELPSCOUT_DOCS_COLLECTION_ID or pass collectionId parameter.');
        }
        return id;
    }
    /**
    ...
    async createDocsArticle(args) {
        ...
        const data = await docsClient.post('/articles', body);
    ...
    async updateDocsArticle(args) {
        ...
        const data = await docsClient.put(`/articles/${input.articleId}`, body);
        const article = data.article || data;
        return {
            content: [{
                    type: 'text',
