...
export const GetAttachmentInputSchema = z.object({
    ...
    format: z.enum(['auto', 'text', 'base64']).default('auto').describe("Output format. 'auto' returns text for text/* or message/rfc822 mime types, base64 otherwise. 'text' forces UTF-8 decode. 'base64' returns raw base64."),
    maxBytes: z.number().int().positive().max(5000000).default(1000000).describe('Safety cap on returned payload size (decoded for text, raw base64 length is ~1.33x this).'),
});
...
export const StructuredConversationFilterInputSchema = z.object({
    ...
    limit: z.number().min(1).max(100).default(50),
    cursor: z.string().optional(),
}).refine((data) => !!(data.assignedTo !== undefined || data.folderId !== undefined || data.customerIds !== undefined || data.conversationNumber !== undefined || (data.sortBy && ['waitingSince', 'customerName', 'customerEmail'].includes(data.sortBy))), { message: 'Must use at least one unique field: assignedTo, folderId, customerIds, conversationNumber, or unique sorting. For content search, use comprehensiveConversationSearch.' });
