...
export interface McpClient {
  client_id: string;
  client_name: string;
  redirect_uris: string[]; // JSON in db
}
...
export function initStore(saasDir: string): void {
...
export function upsertUser(hsUserId: number, email: string, companyId: number | null): SaasUser {
  db.prepare(`
  ...
  return db.prepare('SELECT * FROM users WHERE hs_user_id = ?').get(hsUserId) as SaasUser;
}

export function getUser(userId: number): SaasUser | undefined {
  return db.prepare('SELECT * FROM users WHERE id = ?').get(userId) as SaasUser | undefined;
}

export function saveHsTokens(userId: number, t: HsTokens): void {
  db.prepare(`
...
export function getHsTokens(userId: number): HsTokens | null {
  const row = db.prepare('SELECT * FROM hs_tokens WHERE user_id = ?').get(userId) as
    { access_enc: string; refresh_enc: string; expires_at: number } | undefined;
  if (!row) return null;
  return {
...
export function deleteUserAuth(userId: number): void {
...
export function saveClient(c: McpClient): void {
...
export function getClient(clientId: string): McpClient | undefined {
  const row = db.prepare('SELECT * FROM mcp_clients WHERE client_id = ?').get(clientId) as
...
export function saveGrant(g: Omit<Grant, 'id' | 'revoked' | 'token_hash'> & { token: string }): void {
  db.prepare(`
...
export function consumeGrant(kind: Grant['kind'], token: string): Grant | undefined {
  const row = db.prepare('SELECT * FROM grants WHERE kind = ? AND token_hash = ? AND revoked = 0')
    .get(kind, sha256hex(token)) as Grant | undefined;
  if (!row) return undefined;
...
export function lookupGrant(kind: Grant['kind'], token: string): Grant | undefined {
  const row = db.prepare('SELECT * FROM grants WHERE kind = ? AND token_hash = ? AND revoked = 0')
    .get(kind, sha256hex(token)) as Grant | undefined;
  if (!row || row.expires_at < Math.floor(Date.now() / 1000)) return undefined;
  return row;
}
...
export function saveFlowState(state: string, payload: Record<string, unknown>, ttlSeconds = 600): void {
...
export function consumeFlowState(state: string): Record<string, unknown> | undefined {
  const row = db.prepare('SELECT * FROM hs_flow_state WHERE state = ?').get(state) as
    { payload: string; expires_at: number } | undefined;
...
export function audit(userId: number | null, event: string, detail?: string, durationMs?: number): void {
...
export function rateLimitAllow(userId: number, perMinute: number): boolean {
