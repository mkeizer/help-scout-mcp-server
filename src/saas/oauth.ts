...
import {
  saveClient, getClient, saveGrant, consumeGrant, lookupGrant, revokeToken,
  saveFlowState, consumeFlowState, audit,
} from './store.js';
...
export async function handleOauthRoute(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  try {
    ...
    if (!res.headersSent) sendJson(res, 500, { error: 'server_error', error_description: msg });
    return true;
  }
  return false;
}
...
async function handleHsCallback(url: URL, res: ServerResponse): Promise<void> {
  ...
  if (flow.state) target.searchParams.set('state', String(flow.state));
...
function issueTokens(res: ServerResponse, clientId: string, userId: number, scope: string): void {
 ...
 */
export function authenticateOauthToken(token: string): OauthPrincipal | null {
  if (!token.startsWith('hsmcp_at_')) return null;
  const grant = lookupGrant('access', token);
  if (!grant) return null;
  return { userId: grant.user_id, scope: grant.scope };
}

export function send401WithResourceMetadata(res: ServerResponse): void {
  res.statusCode = 401;
  res.setHeader('content-type', 'application/json');
  res.setHeader(
    'www-authenticate',
    `Bearer resource_metadata="${saasConfig.publicBaseUrl}/.well-known/oauth-protected-resource"`,
  );
