...
import { saveHsTokens, getHsTokens, upsertUser, type SaasUser, type HsTokens } from './store.js';
...
export function buildHsAuthorizeUrl(state: string): string {
...
interface HsTokenResponse {
  access_token: string;
  refresh_token: string;
  expires_in: number;
}

function toTokens(data: HsTokenResponse): HsTokens {
  return {
...
export async function exchangeHsCode(code: string): Promise<HsTokens> {
  const resp = await axios.post<HsTokenResponse>(HS_TOKEN_URL, {
...
export async function refreshHsTokens(userId: number, refreshToken: string): Promise<HsTokens> {
  const resp = await axios.post<HsTokenResponse>(HS_TOKEN_URL, {
...
export async function fetchHsIdentity(accessToken: string): Promise<HsIdentity> {
  const resp = await axios.get('https://api.helpscout.net/v2/users/me', {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  const d = resp.data as { id: number; email: string; companyId?: number; company?: { id?: number }; firstName?: string };
  return {
    hsUserId: d.id,
    email: d.email,
 ...
 */
export async function completeHsLink(code: string): Promise<SaasUser> {
  ...
  if (!isAllowed(identity)) {
    ...
    throw new Error(
      `Account ${identity.email} is not on the access list for this server. ` +
...
function isAllowed(identity: HsIdentity): boolean {
  ...
  if (allow.emails.includes('*')) return true;
  if (identity.companyId !== null && allow.companies.includes(identity.companyId)) return true;
 ...
 */
export async function validHsAccessToken(userId: number): Promise<string | null> {
