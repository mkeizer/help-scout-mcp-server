...
- **Afsluiting op basis van dag + tijdstip** (bepaald via `getServerTime`, geconverteerd naar Amsterdam-tijd CET/CEST):
  - Maandag t/m donderdag vóór 18:00: "Fijne dag!" of een korte succeswens passend bij het onderwerp
  - **Ná 18:00 (elke dag): "Fijne avond!"** — NIET "Fijne dag!" (dag is voorbij), NIET "Goedenavond!" (dat is een begroeting, geen afsluiting)
  - Vrijdagmiddag/-avond (na 12:00): "Alvast een fijn weekend!"
  - Zaterdag: "Fijn weekend!"
  - Zondag: "Fijne zondag!" (vóór 18:00) of "Fijne avond (nog)!" (na 18:00)
  - **Controleer altijd de exacte dag én het uur via `getServerTime`** — reken NIET terug vanuit een eerdere aanroep of aanname. Een verkeerde daggroet ("Fijne zondag" op maandag, of "Fijne dag" om 21:00) valt direct op bij de klant.
...
12. **Afsluiting matcht het uur, niet de dag alleen.** Roep `getServerTime` aan en check ook het uur. **Na 18:00 = "Fijne avond!"**, niet "Fijne dag!" (die is al voorbij) en niet "Goedenavond!" (dat is een begróeting, geen afsluiting — zoals "Hallo!" aan het eind van een brief). Elke avond-ticket dat "Fijne dag!" afsluit is een model-fout die de klant direct opmerkt.
...
### Server access — gateway FIRST, direct SSH almost never

**Direct `ssh -i /home/claude/.ssh/ko-triage ...` is geen standaard tool.** Sinds 2026-04 gebruiken we de `mcp__ssh-gateway__*` MCP voor structured, audit-logged read+write access tot alle cl-servers. Voor >95% van triage-werk is direct SSH niet nodig en niet gewenst.

**Wat beschikbaar is via de gateway** (volledige spec: `.claude/mcp-ssh-gateway.md`, live lijst: `mcp__ssh-gateway__introspect`):

- **File inspection**: `fs_list`, `fs_read` (full/tail/grep/recursive), `fs_hash`, `fs_diff`
- **~44 read templates** via `cmd_run`: `da-user-info`, `webserver-errors`, `cms-inventory`, `wp-cli`, `wp-audit-all`, `cron-audit-all`, `user-cron`, `find-large-files`, `recent-uploads`, `webshell-scan-detect`, `quota-overview`, `user-quota`, `dkim-overview`, `dns`, `dns-zone-read`, `tls-cert-inspect`, `letsencrypt-status`, `mysql-processlist`, `redis-status`, `csf-query`, `imunify`, `processes`, `proc-inspect`, `svc-status`, en meer — zie `.claude/mcp-ssh-gateway.md` voor complete tabel
- **~21 mutate templates** via `cmd_mutate`: `csf-allow/deny`, `da-fix-user`, `exim-queue-action`, `imunify-mutate`, `lve-set-limit`, `service-reload/restart`, `wp-cli-mutate`, enz.
- **DirectAdmin REST API** via `da.read` / `da.mutate` (312 endpoints; `da_endpoints` tool voor discovery)
- **Mail deliverability** (gateway-side, geen SSH): `mail_auth-check`, `mail_fcrdns`, `mail_rbl-check`, `mail_smtp-banner`, `mail_parse-headers`, `mail_rcpt-probe`, `mail_outbound-verify`, `mail_send-count`, `mail_auth-failures`, `mail_forwarder-audit`, `mail_frozen-summary`

**Wanneer WEL direct SSH** (zeldzaam, na expliciete staff-goedkeuring):
1. Gateway template ontbreekt voor wat je nodig hebt — verifieer eerst via `introspect`, flag dan als missing-tool in `runRetrospective.newToolIdea`.
2. Actieve dreiging waar je NU moet ingrijpen en gateway-mutate niet past.
3. Debug-sessie met een mens die live meekijkt.

Volledige regels + SSH-fallback-procedure: zie `.claude/playbooks/ssh-access.md`.
